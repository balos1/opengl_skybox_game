...
std::string ShaderProgram::getInfoLog(ObjectType type, GLuint id)
{
	GLint infoLogLength;
	...
	// output the inforLog if in debug mode
	if (DEBUG)
...
GLuint ShaderProgram::attribute(const std::string attributeName)
{
	...
	static std::map<std::string, GLuint>::const_iterator attributeIter;
...
GLuint ShaderProgram::uniform(const std::string uniformName)
{
	...
	// reuse it for all further calls).
	static std::map<std::string, GLuint>::const_iterator uniformIter;
...
// Method to returns the bound location of a named uniform
GLuint ShaderProgram::texture(const std::string textureName)
{
	// Note: You could do this method with the single line:
	//
	// 		return uniformLocList[uniform];
	...
	// Try to find the named uniform
	uniformIter = textureMap.find(textureName);

	// Found it? Great - pass it back! Didn't find it? Alert user and halt.
	if (uniformIter == textureMap.end())
	{
		throw std::runtime_error("Could not find uniform in shader program: " + textureName);
	}
...
int ShaderProgram::addIndexedUniform(const std::string uniformName, GLuint index)
{
	// Add the uniform location value for the uniformName key
...
int ShaderProgram::addTexture(const std::string textureName)
{
	GLuint texID;
	glGenTextures(1, &texID);

	// Add the texture ID value for the textureName key
	textureMap[textureName] = texID;

	// Check to ensure that the shader contains a texture with this name
	if (textureMap[textureName] == -1)
	{
		throw std::runtime_error("Could not add texture: " + textureName + " - location returned -1.");
	}
	else // Valid texture ID? Inform user if we're in debug mode.
	{
		if (DEBUG)
		{
			std::cout << "Texture " << textureName << "generated with ID : " << textureMap[textureName] << std::endl;
		}
	}
